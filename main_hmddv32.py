...
class GATv2(nn.Module):
    def __init__(self, in_feats, h_feats, fc_out, num_heads):
        ...
        self.mirnafc = nn.Linear(788, fc_out)
        self.diseasefc = nn.Linear(374, fc_out)
    ...
    def forward(self, g, in_feat):
        mirna_feats = in_feat[0:788, :]
        disease_feats = in_feat[788:1163, 0:374]
...
if __name__ == '__main__':
    ...
    (g,), _  = dgl.load_graphs('data/graph_hmdd32_xsd.dgl')
    ...
    for kfold in range(5):
        ...
        model = GATv2(train_g.ndata['feat'].shape[1], h_feats,fc_out,num_heads).to(device) # AUC=0.961
    ...
    print('-----------------------------------------------------------------------------------------------')
    print('epoch:{},'.format(epochs),'num_heads:{}'.format(num_heads))
    print('GAT layer:',)
